...
function StatItem({ label }: { label: string }) {
  return (
    <div className="flex shrink-0 items-center gap-1.5 sm:gap-2">
      <span
        className="size-2.5 shrink-0 rounded-full bg-white sm:size-3.5 dark:bg-[linear-gradient(96.02deg,_#CB7EF7_4.77%,_#FF9BC1_39.51%,_#EE4FAC_74.24%)]"
        aria-hidden
      />
      <span className="text-sm font-semibold leading-normal whitespace-nowrap text-white sm:text-lg dark:text-[#dfe0e4]">
        {label}
      </span>
    </div>
  );
...
export default async function StatsMarqueeSection() {
  ...
  return (
    ...
    >
      ...
      />
      <div className="relative flex w-max motion-safe:animate-[marquee_50s_linear_infinite] motion-safe:sm:animate-[marquee_35s_linear_infinite] items-center gap-6 sm:gap-12">
